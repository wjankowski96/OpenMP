...
float calkowanie(float poczPrzedzial, float koncPrzedzial, int n, int red)
{
    ...
    /* Zapewnienie dostepu do innego bloku pamieci dla kazdego watku*/
     #pragma omp parallel private(xs) //wątek powinien mieć własne wystąpienie zmiennej
        h = (b - a) / (int)lobliczen;            //odleglosc miedzy dwoma sasiednimi punktami, przedzial calkowania dzielony na N rownych czesci
        //#pragma omp for num_threads(lwatkow) reduction(+: calka)//zmiennea, które jest prywatne dla każdego wątku jest obiektem operacji redukcji na końcu regionu równoległego
...
int main()
{
    double poczPrzedzial, koncPrzedzial, dx, calkat, s, x, A, B, C;
     double A1;
    int i, n, red;
